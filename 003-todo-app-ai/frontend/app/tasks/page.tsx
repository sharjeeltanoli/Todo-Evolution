...
export default function TasksPage() {
  ...
  return (
    <div className="min-h-screen bg-[#050505] font-sans text-white overflow-hidden relative selection:bg-indigo-500/30">
      ...
      <main className="container mx-auto p-4 max-w-4xl relative z-10 pt-4">
        ...
        {/* Statistics Bar - Smooth Transition */}
        <div 
          className={`grid grid-cols-1 sm:grid-cols-3 gap-4 overflow-hidden transition-all duration-500 ease-in-out ${
            showCreateForm ? 'max-h-0 opacity-0 mb-0 pointer-events-none scale-95' : 'max-h-[500px] opacity-100 mb-6 scale-100'
          }`}
        >
          {/* Total Tasks */}
          <div 
            onClick={() => setFilter('all')}
            className={`cursor-pointer bg-zinc-900/50 backdrop-blur-md border rounded-xl px-4 py-3 flex items-center justify-between group transition-all duration-300 ${
              filter === 'all' 
          ...
          >
            <div>
              ...
              <p className="text-xl font-black text-amber-400 mt-1 tracking-tight">{tasks.filter(t => !t.completed).length}</p>
            </div>
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center border transition-colors ${
          ...
          >
            <div>
              ...
              <p className="text-xl font-black text-emerald-400 mt-1 tracking-tight">{tasks.filter(t => t.completed).length}</p>
            </div>
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center border transition-colors ${
